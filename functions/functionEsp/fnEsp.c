...
extern int
FnEsp_setApSsid(uint8_t const *buf, ...)
{
    va_list list;
    // buffer for response
    va_start(list, buf);
...
extern int
FnEsp_setApHidden(uint8_t const *buf, ...)
{
    espLogI("%d", *buf);
    wifi_config_t cfg;
    wifi_ap_get_config(&cfg);
...
extern int
FnEsp_setApPass(uint8_t const *buf, ...)
{
    va_list list;
    // buffer for response
    va_start(list, buf);
...
extern int
FnEsp_resetApSettings(uint8_t *buf, ...)
{
    return wifi_ap_reset();
}
//****************	STA SETTINGS	****************
extern int
FnEsp_getStaSettings(uint8_t *buf, ...)
{
    va_list list;
    // buffer for response
    va_start(list, buf);
...
extern int
FnEsp_setStaSettings(uint8_t const *buf, ...)
{
    va_list list;
