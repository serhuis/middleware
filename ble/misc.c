...
void print_uuid(const ble_uuid_t *uuid)
{
    ...
    ESP_LOGD("", "%s:%d: %s", __FILE__, __LINE__, ble_uuid_to_str(uuid, buf));
}
...
void print_conn_desc(const struct ble_gap_conn_desc *desc)
{
    ...
    MODLOG_DFLT(INFO, "role=%d, conn_itvl=%d conn_latency=%d supervision_timeout=%d "
                      "encrypted=%d authenticated=%d bonded=%d key_size=%d",
                desc->role,
...
void print_adv_fields(const struct ble_hs_adv_fields *fields)
{
    ...
    //ESP_LOG_BUFFER_HEXDUMP(__FILE__, fields, sizeof(struct ble_hs_adv_fields), ESP_LOG_INFO);
    ...
    if (fields->name != NULL)
    {
        ...
        ESP_LOGD("", "%s:%d    name(%scomplete)=%s", __FILE__, __LINE__, fields->name_is_complete ? "" : "in", s);
        //ESP_LOG_BUFFER_HEXDUMP(__FILE__, fields->name, fields->name_len, ESP_LOG_INFO);
    }

    if (fields->tx_pwr_lvl_is_present)
    {
        //MODLOG_DFLT(INFO, "    tx_pwr_lvl=%d\n", fields->tx_pwr_lvl);
        ESP_LOGD("", "%s:%d    tx_pwr_lvl=%d", __FILE__, __LINE__, fields->tx_pwr_lvl);
    }
    ...
    if (fields->svc_data_uuid16 != NULL)
    {
        ...
        //MODLOG_DFLT(INFO, "\n");
        ESP_LOG_BUFFER_HEXDUMP(__FILE__, fields->svc_data_uuid16, fields->svc_data_uuid16_len, ESP_LOG_DEBUG);
    }
    ...
    if (fields->svc_data_uuid32 != NULL)
    {
        ...
        MODLOG_DFLT(INFO, "\n");
        //ESP_LOG_BUFFER_HEXDUMP(__FILE__, fields->svc_data_uuid32, fields->svc_data_uuid32_len, ESP_LOG_INFO);
    }
    ...
    if (fields->svc_data_uuid128 != NULL)
    {
        ...
        //MODLOG_DFLT(INFO, "\n");
        ESP_LOG_BUFFER_HEX(__FILE__, fields->svc_data_uuid128, fields->svc_data_uuid128_len);
    }
    ...
    if (fields->mfg_data != NULL)
    {
        ...
        //MODLOG_DFLT(INFO, "\n");
        ESP_LOG_BUFFER_HEXDUMP(__FILE__, fields->mfg_data, fields->mfg_data_len, ESP_LOG_DEBUG);
    }
}
